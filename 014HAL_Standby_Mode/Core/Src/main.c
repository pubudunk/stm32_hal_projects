...
int main(void)
{
	...
	/* 1. Turn on the Clock for backup sram */
	...
	if(__HAL_PWR_GET_FLAG(PWR_FLAG_SB) != RESET) {
		...
		if(data != 'H') {
			Log_Message("Backup SRAM data lost\r\n");
		} else {
			Log_Message("Backup SRAM data retained\r\n");
		}
	} else {

		Log_Message("Woke up normally copying data to backup sram\r\n");
		for(int i = 0 ; i < strlen(pcWritebuf); i++) {
			*(pBackupSRAMBase+i) = pcWritebuf[i];
		}
	}


	Log_Message("Press User button to enter standby mode \r\n");
	...
	/*After wakeup the device will under go a power reset */
